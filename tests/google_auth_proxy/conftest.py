...
That module ships inside the workload image (pushed by
load_superset_files()) rather than imported by the charm, so it is not on
the charm's own PYTHONPATH; it is added to sys.path here instead.
"""
...
def oauth_env(monkeypatch):
    ...
    def _set(**overrides):
        """Set the MCP_AUTH_* variables the charm's _get_mcp_auth_config() would.
