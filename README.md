...
### Connecting to a TLS-enabled JDBC Endpoint

[Kyuubi](https://charmhub.io/kyuubi-k8s) serves its JDBC endpoint over the Thrift *binary* protocol, and enables TLS on it once it is integrated with a certificate provider.

```
hive+tls://<user>:<password>@<host>:10009/<database>
```

Integrate Superset with the same certificate provider as Kyuubi so the CA lands in the workload trust store:

```bash
juju integrate superset-k8s self-signed-certificates
```

The driver accepts three optional query parameters:

| Parameter | Default | Purpose |
| --- | --- | --- |
| `ssl_cert` | system trust store | Path to a PEM bundle to verify the server against. |
| `check_hostname` | `true` | Set to `false` when connecting through a name the server certificate does not carry, such as a Kubernetes `Service` DNS name. Kyuubi only requests SANs for its pod and node. |
| `ssl_verify` | `true` | Set to `false` to skip certificate verification entirely. |
